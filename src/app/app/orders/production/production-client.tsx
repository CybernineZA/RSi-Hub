    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            {orders.length === 0 ? (
              <div className="text-sm text-neutral-400">No production orders yet.</div>
            ) : (
              orders.map((o) => {
                const s = sumOrder(o)
                return (
                  <div key={o.id} className="rounded-xl border border-neutral-800 bg-neutral-900/40 p-4 space-y-3">
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <div className="text-sm text-neutral-200">{o.title}</div>
                        <div className="text-xs text-neutral-500">
                          {formatTs(o.created_at)} • {o.status}
                        </div>
                      </div>
                      <Badge>{s.p}%</Badge>
                    </div>
                      ...
                      {o.order_items.map((it) => (
                        <div key={it.id} className="flex items-center justify-between gap-3">
                          <div className="text-sm text-neutral-300">
                            {it.items?.name ?? 'Item'} — {it.qty_done}/{it.qty_required}
                          </div>
                          <div className="flex items-center gap-2">
                            <Input
                              ...
                              max={it.qty_required}
                              value={it.qty_done}
                              onChange={(e) => setOrderItemDone(o.id, it.id, Number(e.target.value))}
                              disabled={busy}
                            />
                          </div>
                        </div>
